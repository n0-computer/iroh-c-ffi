...
pub fn public_key_as_base32(key: &PublicKey) -> char_p::Box {
    iroh_net::key::PublicKey::from(key)
        .to_string()
        .try_into()
        .unwrap()
}
...
pub fn public_key_from_base32(raw_key: char_p::Ref<'_>, out: &mut PublicKey) -> KeyResult {
    let key: Result<iroh_net::key::PublicKey, _> = raw_key.to_str().parse();
...
impl From<iroh_net::key::PublicKey> for PublicKey {
    fn from(key: iroh_net::key::PublicKey) -> Self {
        PublicKey {
            key: *key.as_bytes(),
        }
    }
...
impl From<PublicKey> for iroh_net::key::PublicKey {
    fn from(key: PublicKey) -> Self {
        iroh_net::key::PublicKey::try_from(&key.key).unwrap()
    }
}
...
pub fn secret_key_default() -> repr_c::Box<SecretKey> {
    Box::new(SecretKey {
        key: iroh_net::key::SecretKey::generate(),
    })
    .into()
}
...
pub fn secret_key_from_base32(
    ...
    let key: Result<iroh_net::key::SecretKey, _> = raw_key.to_str().parse();
...
impl From<iroh_net::key::SecretKey> for SecretKey {
    fn from(key: iroh_net::key::SecretKey) -> Self {
        SecretKey { key }
    }
}
